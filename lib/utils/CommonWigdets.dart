...
ButtonStyle commonButtonStyle(Size size, Color color) {
  return ElevatedButton.styleFrom(
      ...
      elevation: 0.0,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(size.width * numD04)));
}
...
void broadcastDialog({required Size size, required TaskDetailModel taskDetail}) {
  showDialog(
                            ...
                            child: commonElevatedButton(
                              ...
                              Navigator.of(context).push(MaterialPageRoute(
                                  builder: (context) => BroadCastScreen(
                                        ...
                                        mediaHouseId: taskDetail.mediaHouseId,
                                      )));
                            }),
                          ),
                        ),
              ...
              builder: (BuildContext context, StateSetter setState) {
                return Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Padding(
                        padding: EdgeInsets.only(left: size.width * numD04,top:size.width * numD02),
                        child: Row(
                          children: [
                            Text(
                    ...
                    children: [
                      ...
                      Padding(
                        padding: EdgeInsets.only(
                            ...
                            right: size.width * numD04),
                        child: Text(
                          "Please complete your pending onboarding process to register on PRESSHOP",
                          textAlign: TextAlign.left,
                          style: TextStyle(
                              color: Colors.black,
                              fontSize: size.width * numD040,
                              fontWeight: FontWeight.w400),
                        ),
                      ),
                      SizedBox(
...
Future<String?> commonDatePicker({String? date}) async {
  ...
  if (pickedDate != null) {
    final String formatted = dateTimeFormatter(dateTime: pickedDate.toString());
...
String formatMessageTimestamp(DateTime timestamp) {
  ...
  if (difference < const Duration(days: 1)) {
    return DateFormat("HH:mm a").format(timestamp); // Display time
  } else {
    return DateFormat("d MMMM yyyy").format(timestamp); // Display date
  }
}
